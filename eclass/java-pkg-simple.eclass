...
# @ECLASS_VARIABLE: JAVA_INTERNAL_SRC_DIRS
# @DEFAULT_UNSET
# @INTERNAL
# @DESCRIPTION:
# An array accumulating directories containing sources

# @ECLASS_VARIABLE: JAVA_INTERNAL_JAR_FILES
# @DEFAULT_UNSET
# @INTERNAL
# @DESCRIPTION:
# An array accumulating jar files to be installed
...
# @FUNCTION: java-pkg-simple_getmodulesourcepath
# @USAGE: java-pkg-simple_getmodulesourcepath
# @INTERNAL
# @DESCRIPTION:
# Get proper ${modulesourcepath} from provided pathes.
# We use it inside java-pkg-simple_src_compile, java-pkg-simple_do_ejavadoc and
# java-pkg-simple_src_test.
#
# Note that the variable "modulesourcepath" needs to be defined before
# calling this function.
java-pkg-simple_getmodulesourcepath() {
	debug-print-function ${FUNCNAME} $*
...
java-pkg-simple_call_ejavadoc() {
	...
	if ! [[ "${JAVADOC_SRC_DIRS}" ]]; then
		local -a JAVADOC_SRC_DIRS
		JAVADOC_SRC_DIRS=( "${JAVA_INTERNAL_SRC_DIRS[@]}" )
	fi
	# gather sources
	...
	local -a moduleinfo
	readarray -t moduleinfo <<<"$(find "${JAVADOC_SRC_DIRS[@]}" -name module-info.java)"
...
java-pkg-simple_src_compile() {
	...
	if has doc ${JAVA_PKG_IUSE} && use doc; then
		if has binary ${JAVA_PKG_IUSE} && use binary; then
			ewarn "Generation of documentation is not supported when installing binary."
...
# @DESCRIPTION:
# src_install for simple java packages. Simply installs all jars. It
# will also install a launcher if ${JAVA_MAIN_CLASS} is set. Also
# invokes einstalldocs.
java-pkg-simple_src_install() {
	local sources=sources.lst classes=target/classes apidoc=target/api

	# install the jar file that we need
	for JAVA_JAR_FILENAME in "${JAVA_INTERNAL_JAR_FILES[@]}"; do
	...
	if has source ${JAVA_PKG_IUSE} && use source; then
		local srcdirs=""
		if [[ "${JAVA_INTERNAL_SRC_DIRS[@]}" ]]; then
			local parent child
			for parent in "${JAVA_INTERNAL_SRC_DIRS[@]}"; do
